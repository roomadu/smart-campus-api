...
public class DataStore {
    ...
    static {
        ...
        r1.getSensorIds().add(s1.getSensorId());
        r1.getSensorIds().add(s2.getSensorId());
        r2.getSensorIds().add(s3.getSensorId());

        List<SensorReading> hist = new ArrayList<>();
        hist.add(new SensorReading("RD-001", "SENS-TEMP-01", 20.1, "C", System.currentTimeMillis() - 7200000));
        hist.add(new SensorReading("RD-002", "SENS-TEMP-01", 21.0, "C", System.currentTimeMillis() - 3600000));
    ...
    public static void putSensor(Sensor sensor) {
        ...
        if (room != null && !room.getSensorIds().contains(sensor.getSensorId())) {
            room.getSensorIds().add(sensor.getSensorId());
        }
    }
