...
**Technology:** JAX-RS (Jersey 2.35) · Maven Web Application (WAR)  
**Base Path:** `/api/v1`
...
- Room deletion blocked when sensors are still linked → `409 Conflict`
- Sensor creation validates `roomId` existence → `422 Unprocessable Entity`
- Reading POST blocked if sensor status is `MAINTENANCE` → `403 Forbidden`
- Successful reading POST updates parent sensor `currentValue` (side-effect)
...
# 3. Deploy target/smart-campus-api.war to Tomcat/GlassFish/Payara

# 4. Access dashboard
open http://localhost:8080/smart-campus-api/
```
...
# Register sensor (→ 422 - invalid roomId)
curl -s -X POST http://localhost:8080/smart-campus-api/api/v1/sensors \
  -H "Content-Type: application/json" \
...
# Post reading (→ 403 Forbidden - sensor in MAINTENANCE)
